...
def plot_scene_score(scene, t0=None, tN=None, i_trial=None, plot_per_interval=True, save_fig=None):
    plt.figure()
...
def main(folder):
    ...
    for i, scene in enumerate(scenes):
        ...
        plot_scene_depth(scene, plot_all=False, save_fig=out_folder_depth / f"scene{i+1}.png", **kwargs)

    out_folder_scores = folder / "graphs" / "scores"
    out_folder_scores.mkdir(exist_ok=True, parents=True)
    for i, scene in enumerate(scenes):
        print(f"Scene {i+1}/{len(scenes)}")
        plot_scene_score(scene, i_trial=None, save_fig=out_folder_scores / f"scene{i+1}.png")
