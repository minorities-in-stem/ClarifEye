...
class CameraController: NSObject, ObservableObject {
    ...
    weak var captureDataDelegate: CaptureDataReceiver?
...
// MARK: Helper and Data PRocessing Functions
    ...
    extension CameraController {
        ...
        func getObjectClassificationAndDistance(imagePixelBuffer: CVPixelBuffer, depthDataBuffer: CVPixelBuffer) {
            ...
            let request = VNCoreMLRequest(model: detectionModel) { request, error in
                ...
                self.captureDataDelegate?.onClassification(classifications: classifications)
            }
            // Use CPU for Vision processing to ensure that there are adequate GPU resources for rendering.
            request.usesCPUOnly = true
            
            // Use Vision to perform the request on the color image
            let orientation = CGImagePropertyOrientation(captureDataDelegate?.orientation ?? .faceUp)
...
extension CameraController: AVCaptureDataOutputSynchronizerDelegate {
    func dataOutputSynchronizer(_ synchronizer: AVCaptureDataOutputSynchronizer,
        ...
        captureDataDelegate?.onNewData(capturedData: data)
    }
}
